...
<form class="add_new" action='<?php echo base_url();?>exhibition/check_valid/update/<?php echo $part;?>' method='post' name='add_new' enctype='multipart/form-data'>
    ...
    <tr>
        <td colspan="2">   
            <p><b>Материал:</b><input style='border: 1px solid #C0C0C0;width:20%' name='img_material' value="<?php if($result['img_material']){echo $result['img_material'];};?>"/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            <b>Размеры:</b>
                <input style='border: 1px solid #C0C0C0;width:5%' name='img_width' value="<?php echo set_value('img_width',$result['width']);?>"/>&nbsp;x&nbsp;
                <input style='border: 1px solid #C0C0C0;width:5%' name='img_height' value="<?php echo set_value('img_height',$result['height']);?>"/>&nbsp;см&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            <b>Год создания&nbsp;</b><span style="color: #FF0000;font-size: 8pt;">(4 цифры)</span>:
    ...
    <tr>
        <td colspan="2">   
            <p><b>Статус:</b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            <input type="radio" name='status' value="on"<?php echo set_radio('status', 'on', $result['status']=='on'?true:false);?>/>&nbsp;Опубликовано&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            <input type="radio" name='status' value="off"<?php echo set_radio('status', 'off', $result['status']=='off'?true:false);?>/>&nbsp;Скрыто</p>
        </td> 
    </tr> 
    </table>
