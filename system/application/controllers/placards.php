<?php
/**
 * Frontend-Backend controller for placards page
 * 
 * @author Litkovskiy
 * @copyright 2010
 */
...
class Placards extends MY_Controller
{
	...
	public function __construct()
	{
		...
		$this->data_arr['title'] = 'Рекламные плакаты';
...
//$this->firephp->fb($this->data_arr);
	...
	{
	   $this->data_arr['result'] = $this->placards_model->getListJoin();
       foreach($this->data_arr['result'] as $key => $arr){
            $this->data_arr['result'][$key]['count_comment'] = count($this->placards_model->getCountComments($arr['id']));
       }
       $data = array(
                'menu'    => $this->load->view(MENU, '', true), 
                'content' => $this->load->view('placards/show', $this->data_arr, true));
...
//---------------------------------------------------------------    
    /**
	 * show form for new placards
	 */
	public function add_placards()
	{
	   if($this->session->userdata('username'))
        {
    		$data_arr = array(
    	        'menu'    => $this->load->view(MENU_ADMIN, '', true),
    	        'content' => $this->load->view('placards/new', $this->data_arr, true),
...
//---------------------------------------------------------------
	/**
	 * Content of placards for edit
	 */
	public function edit_text()
	{
	   if($this->session->userdata('username'))
        {
            $this->data_arr['result'] = $this->placards_model->getArrWhere('category', array('cat_name' => 'placards'), null, null);
            $data = array(
                'menu'    => $this->load->view(MENU_ADMIN, '', true),
                'content' => $this->load->view('placards/edit', $this->data_arr, true)
                );
...
//---------------------------------------------------------------
	/**
	 * list for edit or (if isset $id) content placards for edit
	 */
	public function edit_placards($id = false)
	{
	   if($this->session->userdata('username'))
        {
    	    $this->data_arr['title'] = 'Редактировать плакат';
            if(!$id)
    		{
    		    $resultArr = $this->placards_model->getArrWhere('category', array('cat_name' => 'placards'), null, null);
                $this->data_arr['result'] = $this->placards_model->getArrWhere('images', array('id_category' => $resultArr[0]['id']), null, null, 'number');
                $data = array(
                   'menu'    => $this->load->view(MENU_ADMIN, '', true),
                   'content' => $this->load->view('placards/edit_placardslist', $this->data_arr, true)
                  );
            ...
            {
                $this->data_arr['result'] = $this->placards_model->getArrWhere('images', array('id' => $id), null, null);
                ...
                $data = array(
                    'menu'    => $this->load->view(MENU_ADMIN, '', true),
                    'content' => $this->load->view('placards/edit_placards', $this->data_arr, true)
                    );
                $this ->load->view('layout_admin', $data);
            }
        }
 ...
 //---------------------------------------------------------------
	 ...
	 */
	public function del_placards($id = false)
	{
	   if($this->session->userdata('username'))
        {
            ...
            if(!$id)
    		{
                ...
                $data = array(
                   'menu'    => $this->load->view(MENU_ADMIN, '', true),
                   'content' => $this->load->view('placards/del_placardslist', $this->data_arr, true)
                  );
...
//---------------------------------------------------------------	
	...
	/**
     * Check validation of data placardss_page and do update
     */
...
//---------------------------------------------------------------	
	...
	public function check_valid($action)
	{
	    if($this->session->userdata('username'))
        {
            ...
            if($action == 'add')
    		{
    		  $category_arr = $this->placards_model->getArrWhere('category',array('cat_name' => 'placards'), null, null);
              ...
              elseif($count > 0)
              {
                ...
                for($k=0; $k<=$countArr; $k++)
                {
                    ...
                    {
        			    $upload_path = base_url()."img/placards/";
                        $img_arr     = array();
                        ...
                        {
                            $max_number = $this->placards_model->getMaxNumByIdCat('images', $category_arr[0]['id']);
                           
                            $data ['id_category']  = $category_arr[0]['id'];
                            $data ['number']       = $max_number[0]['number']+1;
                            $data ['img_path']     = "placards/".$filename;
                ...
                $this->data_arr['result'] = $this->placards_model->getArrWhere('images', array('id_category' => $category_arr[0]['id']), null, null, 'number');
                $data = array(
        		   'menu'    => $this -> load -> view(MENU_ADMIN,'',true),
            	   'content' => $this->load->view('placards/edit_placardslist', $this->data_arr, true),
                   'title'   => $this->data_arr['title']
...
//---------------------------------------------------------------
	 ...
	 * Content for sort
     * 
     * @param string get name of placards's part
	 */
	   ...
	   if($this->session->userdata('username'))
        {
            ...
            $resultArr                = $this->photos_model->getArrWhere('category',array('cat_name' => 'placards'), null, null);
            $this->data_arr['result'] = $this->photos_model->getArrWhere('images',array('id_category' => $resultArr[0]['id']), null, null, 'number');
            $data = array(
               'menu'    => $this->load->view(MENU_ADMIN, '', true),
               'content' => $this->load->view('placards/edit_list_sortable', $this->data_arr, true)
              );
...
//---------------------------------------------------------------
    /**
	 * add the new placards information
     * 
	...
	{  
	    if($plural)
        {
		   if($this ->placards_model->addInTable($data, $table))
           {
                return true;
           }
           else
        ...
        {
            ($this ->placards_model->addInTable($data, $table)) ? $this->data_arr['message'] = MESS_SUCCES : $this->data_arr['message'] = MESS_ERROR;
            
            $this->data_arr['result'] = $this->placards_model->getArrWhere('images', array('id_category' => $data['id_category']), null, null, 'number');
            $data = array(
    		   'menu'    => $this -> load -> view(MENU_ADMIN,'',true),
        	   'content' => $this->load->view('placards/edit_placardslist', $this->data_arr, true),
               'title'   => $this->data_arr['title']
...
//---------------------------------------------------------------    
	...
	private function _update($data, $table=null)
	{
	   ...
	   if($table)
       {
            ($this->placards_model->updateInTable($_POST['id'], $data, $table)) ? $this->data_arr['message'] = MESS_SUCCES : $this->data_arr['message'] = MESS_ERROR;
       }
       else
       {
            //update text
            unset($data['id_category']);
            ($this->placards_model->update($_POST['id'], $data)) ? $this->data_arr['message'] = MESS_SUCCES : $this->data_arr['message'] = MESS_ERROR;
       }
        $this->data_arr['result'] = $this->placards_model->getArrWhere('images', array('id_category' => $id_category), null, null, 'number');
		$data_arr = array(
	        'menu'    => $this->load->view(MENU_ADMIN,'',true),
	        'content' => $this->load->view('placards/edit_placardslist', $this->data_arr, true),
            'title'   => $this->data_arr['title']
