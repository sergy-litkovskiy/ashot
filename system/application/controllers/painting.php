...
class Painting extends MY_Controller
{
	 ...
	 */
	public function surrealism()
	{   
       $this->data_arr['result']    = $this->painting_model->getListJoin('surrealism');
	...
	/**
	 * Creating sundry page
	 */
	public function sundry()
	{   
       $this->data_arr['result']    = $this->painting_model->getListJoin('sundry');
       foreach($this->data_arr['result'] as $key => $arr){
            $comments = $this->painting_model->getComments($arr['id']);
            $this->data_arr['result'][$key]['comments'] = $comments;
            $this->data_arr['result'][$key]['count_comment'] = count($comments);
...
//---------------------------------------------------------------	
	...
	{
       $this->data_arr['result']    = $this->painting_model->getListJoin('landscapes');
       foreach($this->data_arr['result'] as $key => $arr){
            $comments = $this->painting_model->getComments($arr['id']);
            $this->data_arr['result'][$key]['comments'] = $comments;
            $this->data_arr['result'][$key]['count_comment'] = count($comments);
...
//---------------------------------------------------------------
	...
	/**
	 * Creating still_life page
	 */
	public function still_life()
	{
       $this->data_arr['result']    = $this->painting_model->getListJoin('still_life');
       foreach($this->data_arr['result'] as $key => $arr){
            $comments = $this->painting_model->getComments($arr['id']);
            $this->data_arr['result'][$key]['comments'] = $comments;
            $this->data_arr['result'][$key]['count_comment'] = count($comments);
...
//---------------------------------------------------------------
	...
	public function comment_add()
	{
        ...
        if ($this->form_validation->run() !== false)
		{
         ...
         redirect(base_url().'painting/'.$_POST['part_name']);
	}
//------------------------------------------------------------------
...
//---------------------------------------------------------------
	...
	public function sendMailMess($data)
	{
        ...
        $subject      = "����� �� ����� ashot.kiev.ua";
		$message      = "���� ��������: ".$date.".\r\n �� ����� ��� �������� ����� �� ���������� ' ".$author." ' (email ����������� : ".$email_from.$phone.").\r\n
		����� ������: \r\n".$text;
